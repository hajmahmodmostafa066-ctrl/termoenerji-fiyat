...
import { Eye, Plus, Edit, Trash2, Save, X, Search, Calendar, User, Clock, Shield, Building2 } from 'lucide-react'
...
export default function FirmalarPage() {
  ...
  const handleDetayGoster = async (firma) => {
    setSecilenFirma(firma)
    setModalAcik(true)

    if (isAdmin) {
      const logs = await getAuditLogs(firma.id, 'firmalar')
      setAuditLogs(logs)
    }
  }
  ...
  const handleEkle = async () => {
    ...
    try {
      ...
      if (data && data[0]) {
        await addAuditLog('firmalar', data[0].id, 'INSERT', null, data[0])
      }
  ...
  const handleSil = async (id, ad) => {
    ...
    try {
      await addAuditLog('firmalar', id, 'DELETE', { ad }, null)
  ...
  const handleDuzenleKaydet = async (id) => {
    ...
    try {
      const eskiVeri = firmalar.find(f => f.id === id)
      ...
      await addAuditLog('firmalar', id, 'UPDATE', eskiVeri, yeniVeri)
      ...
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-slate-800 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 border border-slate-700/50 shadow-2xl">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div>
              <p className="text-xs text-slate-400">Firma Adı</p>
              <p className="text-white font-medium">{secilenFirma.ad}</p>
            </div>
            <div>
              <p className="text-xs text-slate-400">Oluşturulma</p>
              <p className="text-white font-medium flex items-center gap-1">
                <Calendar className="h-3 w-3 text-slate-400" />
                {formatDate(secilenFirma.created_at)}
              </p>
            </div>
          </div>
            ...
            <div className="border-t border-slate-700 pt-4">
              ...
              {auditLogs.length === 0 ? (
                <p className="text-sm text-slate-400 text-center py-4">
                  Bu firma için henüz aktivite kaydı yok
                </p>
              ) : (
                    ...
                    <div key={log.id} className="bg-slate-700/30 rounded-lg p-3 border border-slate-600/30">
                      ...
                      {log.islem_turu === 'INSERT' && log.yeni_veri && (
                        <div className="mt-1 text-xs text-emerald-400">
                          Yeni firma oluşturuldu: {log.yeni_veri.ad}
                        </div>
                      )}
                      {log.islem_turu === 'DELETE' && log.eski_veri && (
                        <div className="mt-1 text-xs text-red-400">
                          Firma silindi: {log.eski_veri.ad}
                        </div>
                      )}
                    </div>
                  ))}
