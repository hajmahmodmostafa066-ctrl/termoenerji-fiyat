...
export default function PanelPage() {
  ...
  const StatCard = ({ title, value, icon: Icon, color, change, loading }) => {
    const bg = color + '20'
    return (
      <div className="card hover:border-opacity-50 transition-all duration-300 relative overflow-hidden group">
          ...
          <div>
            ...
            {change !== undefined && (
              <div className="flex items-center gap-1 mt-3">
                ...
                <span className="text-xs font-medium" style={{ color }}>{Math.abs(change)}%</span>
                <span className="text-xs text-slate-500">geçen hafta</span>
              </div>
            )}
          </div>
          <div className="p-3 rounded-xl transition-all duration-300 group-hover:scale-110" style={{ backgroundColor: bg }}>
  ...
  const MenuItem = ({ title, icon: Icon, href, count, color, onClick }) => {
    const bg = color + '20'
    return (
      <div onClick={onClick} className="flex items-center justify-between p-3 rounded-xl transition-all duration-200 cursor-pointer hover:bg-slate-700/30 border border-slate-700/30 hover:border-opacity-50" style={{ borderColor: color + '40' }}>
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-lg transition-all duration-300 group-hover:scale-110" style={{ backgroundColor: bg }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <ChevronRight className="h-4 w-4 text-slate-500" style={{ color }} />
        </div>
      </div>
    )
  }
  ...
  const TeklifCard = ({ teklif, onClick }) => {
    const durum = teklif.durum || 'pending'
    const durumColors = { approved: '#10b981', pending: '#eab308', rejected: '#ef4444' }
    const durumLabels = { approved: '✅ Aktif', pending: '⏳ Beklemede', rejected: '❌ Pasif' }
    const color = durumColors[durum] || '#eab308'
    const bg = color + '20'
    return (
      <div onClick={onClick} className="flex items-center justify-between p-3 rounded-xl border border-slate-700/30 hover:bg-slate-700/20 transition-all duration-200 cursor-pointer">
        ...
        <div className="flex items-center gap-3 flex-shrink-0 ml-4">
          ...
          <span className="text-xs px-2 py-0.5 rounded-full border whitespace-nowrap" style={{ color, backgroundColor: bg, borderColor: color }}>
            {durumLabels[durum] || '⏳ Beklemede'}
          </span>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        {/* HEADER */}
        <div className="card bg-slate-800/30 border-slate-700/50 rounded-3xl p-6">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-xs text-slate-400 flex items-center gap-1">
                    <Sparkles className="h-3 w-3 text-emerald-400" /> Ultra Pro Max Yönetim Paneli
                  </span>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 text-xs text-slate-400 bg-slate-800/50 px-3 py-1.5 rounded-full border border-slate-700/50">
                <Zap className="h-4 w-4 text-emerald-400 animate-pulse" /> Sistem Aktif
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard title="Firmalar" value={stats.toplamFirma} icon={Building2} color="#f59e0b" loading={loading} />
          <StatCard title="Kategoriler" value={stats.toplamKategori} icon={Layers} color="#8b5cf6" loading={loading} />
        </div>
