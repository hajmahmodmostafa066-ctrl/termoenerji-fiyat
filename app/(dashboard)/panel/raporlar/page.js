      ...
      <Page size="A4" style={pdfStyles.page}>
        ...
        <View style={pdfStyles.header}>
          <View style={pdfStyles.headerLeft}>
            ...
            {firmaBilgileri?.web && <Text style={pdfStyles.companySub}>🌐 {firmaBilgileri.web}</Text>}
          </View>
          <View style={pdfStyles.headerRight}>
            ...
            <Text style={pdfStyles.dateText}>💰 Para Birimi: {paraBirimi} ({getCurrencySymbol(paraBirimi)})</Text>
        ...
        <View style={pdfStyles.filterContainer}>
          ...
          <View style={pdfStyles.filterTag}>
            <Text style={pdfStyles.filterTagText}>
              📅 <Text style={pdfStyles.filterTagBold}>{formatDateShort(new Date())}</Text>
            </Text>
          </View>
        </View>
...
export default function RaporlarPage() {
  ...
  // ... (önceki useEffect'ler aynı)

  // Fiyat istatistiklerini hesapla
  useEffect(() => {
    const fiyatlarTL = filteredFiyatlar.map(item => {
  ...
  }, [filteredFiyatlar, gorunenParaBirimi, kurlar])

  // Seçili verileri etiketlerle hazırla
  const getPreparedData = () => {
    return filteredFiyatlar.filter(item => seciliIds.includes(item.id)).map(item => {
      ...
      return {
        ...item,
        _etiket: etiket,
        _convertedPrice: convertedPrice.convertedValue,
      }
    })
  }
